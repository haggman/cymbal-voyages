...
#            Parquet and trains the propensity model. It does NOT re-score
#            customers (decision 2026-09-19, see section 5).
...
# existing definition), then train the BQML model (56 s measured).
#
# 🔴 DECIDED 2026-09-19 (Patrick): NO re-scoring. predict_propensity.sql is NOT
# run. customer_features keeps the propensity_score it ships with (the reference
# model in generate_warehouse.py), because every number the lab quotes was
# verified end to end on those scores: base audience 3,838 at 0.156 ("0.16"),
# 289 send_offer / 14 loyalty / 3,535 held, 1,082 at the 0.15 threshold.
# Measured the same day with re-scoring ON: 336 / 15 / 3,487, and the 12–24
# month variant rounds to 0.20 instead of 0.19 — every Task 3 and Task 6 number
# would have had to change. The BQML model is still trained so it exists in the
# project for anyone who looks; it agrees with the reference scores to ~0.01.
