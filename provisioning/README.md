...
| 6 | Cloud Run `audience-tools` (Toolbox 1.12.0, `tools.yaml` from secret `audience-tools-config`) and `orchestrator` (1.0.1, with Gemini retry/backoff; `GOOGLE_CLOUD_LOCATION=global`, `AGENT_URL` = its own deterministic URL). Both min instances 1, private, `run.invoker` for the Discovery Engine agent | `google_cloud_run_v2_service`, after a 60 s IAM settle | ~1–2 min |
