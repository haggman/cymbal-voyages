...
- Image generation: assistant and Chat agent both generate; Workflow errors. Model name to record on the run-through.
...
- **R4 (Sep 24 recheck list):** the Skill + agent restriction; the Actions tab layout; where Google Search's default sits in the Chat agent's tools picker, and whether Task 0.3's app-level default already covers it; the banned-phrase re-run with the tightened rule; `variant_performance` called before generation; workflow follow-up behaviour (only if a Workflow is used anywhere, which it currently isn't); the A2A card fields Gemini Enterprise rejects.
